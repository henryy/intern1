...
%Add a pseudorandom number generator (PRNG)
xMin = 2^8;
xMax = 2^11;
%PRNG = randi([xMin xMax-1],2,2)
%
PRNG = [1879 1892 ; 483 1389];
PEssai=[1800 1900 2000 2100 0]

a = 1;
%PRNGBin=de2bi(PRNG,12)

%xlswrite('myGenerator.xlsx', PRNGBin);
%xlsread('myGenerator.xlsx','A1:L4');
...
A = int8(A);
    ...
    % decomposing the image in 8x8 block 
       ...
       %Ydec=PRNG(itemp:itemp,jtemp:jtemp)
       % Ybin=dec2bin(PRNG(itemp:itemp,jtemp:jtemp),12)
    ...
    PrngSelectedForXor = dec2bin(PEssai(a),12) 
    for indexSelectedCoef = 1:6
        %xor with function myxor
        
       coefSelectedRoundAbsBinXor(indexSelectedCoef,1:12) = myXor(coefSelectedRoundAbsBin(indexSelectedCoef,1:12), PrngSelectedForXor);
...
for jtemp = 1 : 2
   for itemp = 1 : 2
    ...
    coefRecoverRoundAbsBin
      ...
      %Ynum=bi2de(PRNGBin(indiceBlock,:))
      %Ybin=dec2bin(PRNG(1),12);
    ...
    for indexSelectedCoef = 1:6
        %xor with function myxor
       coefRecoverRoundAbsBinXor(indexSelectedCoef,1:12) = myXor(coefRecoverRoundAbsBin(indexSelectedCoef,1:12), PrngSelectedForXor);
       %put binary in decimal
       %initialisation (without the sign)
