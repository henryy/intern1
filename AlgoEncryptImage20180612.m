...
for jtemp = 1 : (S/(W2*L2))
    for itemp = 1 : (S/(W2*L2))
       ...
       %initialization (without the sign)
...
%recover = uint8(imgProtected);
recover = imgProtected;
columnRecover = 1;
imgRecover1 = uint8(zeros(512,512));
...
for jtemp = 1 : (S/(W2*L2))
   for itemp = 1 : (S/(W2*L2))
    ...
    %transform with integer
        ...
        %imgRecover1((itemp-1)*8+1 : (itemp-1)*8+8, (jtemp-1)*8+1 : (jtemp-1)*8+8) = uint8(tempRecoverIdctInt);
...
subplot (1,3,2); imshow(uint8(imgProtected)); title('encrypted image');
